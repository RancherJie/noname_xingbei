...
export class Create {
	...
	characterDialogOL() {
		...
		//     }
		// }
		var filter, str, noclick, thisiscard, seperate, expandall, onlypack, heightset, characterx, characterList;
		for (var i = 0; i < arguments.length; i++) {
			if (arguments[i] === "thisiscard") {
				thisiscard = true;
			} else if (arguments[i] === "expandall") {
		...
		if (thisiscard) {
			...
			for (var i in lib.character) {
				if (characterList && !characterList.includes(i)) continue;
				list.push(i);
				if (get.is.double(i)) {
					groups.add("double");
				} else groups.add(lib.character[i][1]);
		...
		if (!thisiscard) {
			...
			var packlist = lib.configOL.characterPack;
	 ...
	 */
	zhanJi() {
		game.addVideo('createZhanJi',null);
